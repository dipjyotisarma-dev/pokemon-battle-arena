...
def continue_current_battle(
    ...
    return BattleContinueResponse(
        ...
        trainer_max_hp=match_state["trainer_max_hp"],
...
@router.post(
    "/{battle_id}/move",
    response_model=BattleMoveResponse,
)
def execute_move(
    battle_id: str,
    ...
    """
    Execute the trainer's selected move and the AI's response.
    """
    try:
        battle = execute_trainer_move(
            db=db,
            battle_id=battle_id,
            trainer_id=current_user.id,
    ...
    return BattleMoveResponse(
        ...
        trainer_current_hp=state["trainer_current_hp"],
